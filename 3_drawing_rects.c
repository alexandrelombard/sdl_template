...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        SDL_SetRenderDrawColor(renderer, 0, 0, 0, SDL_ALPHA_OPAQUE);    // Everything after will be drawn in black (RGB: 0, 0, 0)
        SDL_RenderClear(renderer);      // We clear the screen with the render draw color (here, black)

        SDL_SetRenderDrawColor(renderer, 255, 255, 255, SDL_ALPHA_OPAQUE);  // We change the render draw color to white

        // We create a first rectangle and we draw it full
        SDL_Rect r1;
        r1.x = 30;
        r1.y = 60;
        r1.w = 100;
        ...
        SDL_SetRenderDrawColor(renderer, 255, 0, 0, SDL_ALPHA_OPAQUE);  // We change the render draw color to red

        // We create a second rectangle and we draw it full
        SDL_Rect r2;
        r1.x = 40;
        r1.y = 80;
        r1.w = 200;
